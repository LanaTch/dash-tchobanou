...
app.layout = html.Div([
    html.H2(children="Интерактивный дашборд. Статистика по играм.", style={'padding': '10px 5px 5px 5px', "fontSize": "32px"}), #заголовок    
    
    html.Label("Предназначен для обзора статистики по играм, выпущенным с 2000 по 2016 год."
                                "Фильтры по жанрам и рейтингам поддерживают множественный выбор, фильт по годам выпуска (внизу страницы) интервальный.",
...
def update_data(genre_values, 
    ...
    for cur_plat in list(data.Platform.unique()):
        ...
        fig1.add_trace(go.Scatter(
            x=data.query('Platform == @cur_plat')['Year_of_Release'].values, y=data.query('Platform == @cur_plat')['num'].values,
            mode='lines',
            line=dict(width=0.5),
            name=cur_plat,
            text=data['Platform'],
